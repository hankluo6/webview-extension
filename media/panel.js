...
painterElement?.addEventListener('mousemove', function(e) {
	var color = /** @type {HTMLInputElement} */ (document.getElementById('hex'))?.value;
	if (e.buttons != 1)
		return;
	context?.beginPath();
	context?.moveTo(lastPos.x - this.offsetLeft, lastPos.y - this.offsetTop);
	setPosition(e);
	context?.lineTo(lastPos.x - this.offsetLeft, lastPos.y - this.offsetTop)
